...
class TestPublicPriorLoader(unittest.TestCase):
    ...
    def test_02_visibility_must_be_exactly_public(self):
        for value in ("private", "closed", "Public", ""):
            with self.subTest(value=value):
                self.assertIn(
                    "visibility must be exactly 'public'",
                    _error_text(_document(visibility=value)),
                )

    def test_03_unsupported_schema_version_is_rejected(self):
        self.assertIn(
            "unsupported schema version 2", _error_text(_document(schema_version=2))
        )
        ...
        species = _species(
            variants=[
                ...
                _variant(
                    "physical",
                    item_id="none",
                    move_ids=["quickattack", "irontail", "nuzzle", "protect"],
                ),
            ]
        )
        record = _load_document(_document(species=[species])).get_species("pikachu")
        self.assertEqual(
            ("physical", "special"), tuple(v.variant_id for v in record.variants)
        )
    ...
    def test_08_variant_order_is_deterministic(self):
        ...
        second = _load_document(
            _document(species=[_species(variants=list(reversed(variants)))])
        )
        self.assertEqual(
            tuple(v.variant_id for v in first.species[0].variants),
            tuple(v.variant_id for v in second.species[0].variants),
        )
    ...
    def test_09_species_order_is_deterministic(self):
        ...
        self.assertEqual(
            ("pikachu", "raichu"),
            tuple(record.species_id for record in dataset.species),
        )
    ...
    def test_10_exact_forms_remain_distinct(self):
        ...
        self.assertIsNot(
            dataset.get_species("rotomwash"), dataset.get_species("rotomheat")
        )
        self.assertIsNone(dataset.get_species("rotom"))

    def test_11_dudunsparce_forms_remain_distinct(self):
        two = _species(
            "dudunsparce", variants=[_variant(base_ability_id="serenegrace")]
        )
        three = _species(
            "dudunsparcethreesegment",
            base_species_id="dudunsparce",
            variants=[_variant(base_ability_id="serenegrace")],
        )
    ...
    def test_12_public_dataset_identity_is_deterministic(self):
        ...
        self.assertEqual(
            PublicPriorIdentity("syntheticprior", "1.0", "gen9tugs"), identity
        )
        self.assertEqual(
            hash(identity),
            hash(PublicPriorIdentity("syntheticprior", "1.0", "gen9tugs")),
        )
    ...
    def test_13_registry_holds_multiple_datasets(self):
        ...
        self.assertIsNotNone(
            registry.get(PublicPriorIdentity("secondsyntheticprior", "2.0", "gen9tugs"))
        )
    ...
    def test_16_species_lookup_is_exact_form_only(self):
        dataset = _load_document(
            _document(
                species=[
                    _species(
                        "rotomwash", variants=[_variant(base_ability_id="levitate")]
                    )
                ]
            )
        )
    ...
    def test_28_empty_dataset_is_rejected(self):
        self.assertIn(
            "at least one exact species prior", _error_text(_document(species=[]))
        )
    ...
    def test_30_duplicate_source_id_is_rejected(self):
        ...
        self.assertIn(
            "duplicate source ID",
            _error_text(_document(sources=[source, source.copy()])),
        )

    def test_31_noncanonical_source_id_is_rejected(self):
        source = {"source_id": "Synthetic-Fixture", "kind": "synthetic_test"}
        self.assertIn(
            "expected canonical ID 'syntheticfixture'",
            _error_text(_document(sources=[source])),
        )
    ...
    def test_32_unsupported_and_private_source_kinds_are_rejected(self):
        private_kinds = (
            ...
            "submitted_team",
            "private_replay",
            "trainer_private_data",
        )
        for kind in private_kinds:
            with self.subTest(kind=kind):
                source = {"source_id": "syntheticfixture", "kind": kind}
                self.assertIn(
                    "unsupported source kind", _error_text(_document(sources=[source]))
                )
    ...
    def test_33_unknown_source_reference_from_variant_is_rejected(self):
        ...
        self.assertIn(
            "unknown public source reference", _error_text(_document(species=[species]))
        )

    def test_34_duplicate_source_reference_is_rejected(self):
        species = _species(
            variants=[_variant(source_ids=["syntheticfixture", "syntheticfixture"])]
        )
        self.assertIn(
            "duplicate source reference", _error_text(_document(species=[species]))
        )

    def test_35_unknown_species_form_is_rejected(self):
        self.assertIn(
            "unknown exact species/form ID",
            _error_text(_document(species=[_species("syntheticunknownmon")])),
        )
    ...
    def test_37_base_species_mismatch_is_rejected(self):
        species = _species(
            "rotomwash",
            base_species_id="pikachu",
            variants=[_variant(base_ability_id="levitate")],
        )
        self.assertIn(
            "must match authoritative base species 'rotom'",
            _error_text(_document(species=[species])),
        )

    def test_38_empty_species_variant_collection_is_rejected(self):
        self.assertIn(
            "at least one public variant",
            _error_text(_document(species=[_species(variants=[])])),
        )
    ...
    def test_40_noncanonical_variant_id_is_rejected(self):
        self.assertIn(
            "expected canonical ID 'standardset'",
            _error_text(
                _document(species=[_species(variants=[_variant("Standard-Set")])])
            ),
        )
    ...
    def test_41_zero_weight_is_rejected(self):
        self.assertIn(
            ...
            _error_text(_document(species=[_species(variants=[_variant(weight=0)])])),
        )

    def test_42_negative_weight_is_rejected(self):
        self.assertIn(
            "strictly greater than zero",
            _error_text(_document(species=[_species(variants=[_variant(weight=-1)])])),
        )
    ...
    def test_43_nan_weight_is_rejected(self):
        self.assertIn(
            "finite",
            _error_text(
                _document(species=[_species(variants=[_variant(weight=float("nan"))])])
            ),
        )
        ...
        for value in (float("inf"), float("-inf")):
            with self.subTest(value=value):
                self.assertIn(
                    "finite",
                    _error_text(
                        _document(species=[_species(variants=[_variant(weight=value)])])
                    ),
                )
    ...
    def test_45_boolean_weight_is_rejected(self):
        self.assertIn(
            "expected a number",
            _error_text(
                _document(species=[_species(variants=[_variant(weight=True)])])
            ),
        )
    ...
    def test_46_positive_finite_weights_are_accepted(self):
        ...
        record = _load_document(
            _document(species=[_species(variants=variants)])
        ).get_species("pikachu")
        self.assertEqual(
            (1e300, 0.001), tuple(variant.weight for variant in record.variants)
        )

    def test_47_derived_normalized_probabilities_sum_to_one(self):
        variants = [_variant("alpha", weight=1), _variant("beta", weight=3)]
        probabilities = _load_document(
            _document(species=[_species(variants=variants)])
        ).normalized_probabilities("pikachu")
        self.assertTrue(
            math.isclose(1.0, sum(entry.probability for entry in probabilities))
        )
        self.assertEqual(
            (0.25, 0.75), tuple(entry.probability for entry in probabilities)
        )
        with self.assertRaises(FrozenInstanceError):
    ...
    def test_48_normalization_does_not_mutate_source_weights(self):
        dataset = _load_document(
            _document(species=[_species(variants=[_variant("a", 2), _variant("b", 5)])])
        )
    ...
    def test_49_exactly_four_moves_are_required(self):
        for moves in (
            ["protect"],
            ["protect", "surf", "voltswitch", "thunderbolt", "nuzzle"],
        ):
    ...
    def test_50_duplicate_moves_are_rejected(self):
        species = _species(
            variants=[_variant(move_ids=["protect", "protect", "surf", "thunderbolt"])]
        )
        self.assertIn(
            "duplicate move ID 'protect'", _error_text(_document(species=[species]))
        )
    ...
    def test_51_unknown_move_is_rejected(self):
        species = _species(
            variants=[
                _variant(
                    move_ids=["protect", "surf", "thunderbolt", "syntheticunknownmove"]
                )
            ]
        )
    ...
    def test_52_noncanonical_move_is_rejected_not_rewritten(self):
        species = _species(
            variants=[
                _variant(move_ids=["Protect", "surf", "thunderbolt", "voltswitch"])
            ]
        )
        self.assertIn(
            "expected canonical ID 'protect', received 'Protect'",
            _error_text(_document(species=[species])),
        )

    def test_53_invalid_ability_is_rejected(self):
        species = _species(
            variants=[_variant(base_ability_id="syntheticunknownability")]
        )
        self.assertIn(
            "base ability does not belong", _error_text(_document(species=[species]))
        )
    ...
    def test_55_ambiguous_no_item_representation_is_rejected(self):
        ...
        dataset = _load_document(
            _document(species=[_species(variants=[_variant(item_id="none")])])
        )
        self.assertEqual("none", dataset.species[0].variants[0].item_id)

    def test_56_known_nature_is_accepted(self):
        dataset = _load_document(
            _document(species=[_species(variants=[_variant(nature_id="adamant")])])
        )
    ...
    def test_59_ev_ranges_are_validated(self):
        ...
        self.assertGreaterEqual(
            _error_text(_document(species=[species])).count("between 0 and 252"), 2
        )
    ...
    def test_60_ev_total_is_validated(self):
        ...
        species = _species(variants=[_variant(evs=evs)])
        self.assertIn(
            "total must not exceed 510", _error_text(_document(species=[species]))
        )
    ...
    def test_61_iv_field_names_are_validated(self):
        ...
        self.assertIn(
            "unexpected stat field", _error_text(_document(species=[species]))
        )
    ...
    def test_62_iv_ranges_are_validated(self):
        ...
        species = _species(variants=[_variant(ivs=ivs)])
        self.assertGreaterEqual(
            _error_text(_document(species=[species])).count("between 0 and 31"), 2
        )
    ...
    def test_63_level_range_is_validated(self):
        for level in (0, 101):
            with self.subTest(level=level):
                ...
                self.assertIn(
                    "between 1 and 100", _error_text(_document(species=[species]))
                )
    ...
    def test_65_multiple_independent_errors_are_aggregated(self):
        variant = _variant(
            weight=0,
            move_ids=["Unknown Move"],
            nature_id="unknownnature",
            level=101,
            source_ids=["missing"],
        )
        error = _validation_error(
            _document(
                visibility="closed",
                species=[_species("unknownmon", variants=[variant])],
            )
        )
    ...
    def test_77_no_battle_or_search_module_is_a_loader_dependency(self):
        ...
        with mock.patch.object(
            ...
            _load_document()
        self.assertFalse(
            {name for name in set(sys.modules) - before if name.startswith("fp.search")}
        )
    ...
    def test_80_registry_iteration_and_cross_dataset_lookup_are_deterministic(self):
        ...
        references = registry.references_for_species(
            "pikachu", (second.identity, first.identity)
        )
        self.assertEqual(tuple(sorted(references)), references)
        self.assertEqual(
            {first.identity, second.identity},
            {reference.dataset_identity for reference in references},
        )
        self.assertEqual(
            (2.0,),
            tuple(variant.weight for variant in first.get_species("pikachu").variants),
        )
